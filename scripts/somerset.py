...
def run():
    ...
    with open(DATA_SOURCE, 'r') as som_raw:
        ...
        for row in mobreader:
            ...
            mobile = [route, stop, community, address, easting, northing, longitude,
                      latitude, date.strftime('%Y-%m-%d'), day, frequency, start, end, timetable]
    ...
    with open(DATA_OUTPUT, 'w', encoding='utf8', newline='') as out_csv:
        ...
        mob_writer.writerow(
            ['Route', 'Stop', 'Community', 'Address', 'Easting', 'Northing', 'Longitude',
             'Latitude', 'Date', 'Day', 'Frequency', 'Start', 'End', 'Timetable'])
        for stop in mobiles:
            mob_writer.writerow(
                [stop[0], stop[1], stop[2], stop[3], stop[4], stop[5], stop[6],
                 stop[7], stop[8], stop[9], stop[10], stop[11], stop[12], stop[13]])
